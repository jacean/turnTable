...
var subradiu=25;
    ...
    function loadCenterDom(center) {
		...
		turn_center.css('width',center_r*2).css('height',center_r*2)
        turn_center.append(
    ...
    function initTable() {
         ...
         * 设置中心点
         * turn-table
         * width(left)=subradius+radius*2+subradius
         */
        if ($.isEmptyObject(coord)) {
            
            if (typeof options.sub.radius != "undefined") {
                subradiu=options.sub.radius
            }
                xcenter = radius + subradiu;
                ycenter = radius + subradiu;
